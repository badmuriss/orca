...
vi.mock('@/runtime/runtime-maestro-client', () => ({
  captureMaestroBrowserSurfacePreview: (...args: unknown[]) => capture(...args)
}))

const { MaestroBrowserSurfaceCard } = await import('./MaestroBrowserSurfaceCard')

function receiptFor(updatedAt: string) {
  return MaestroBrowserSurfaceReceiptSchema.parse({
    ...
    workspace_key: 'worktree:repo-1::/tmp/repo',
...
describe('MaestroBrowserSurfaceCard preview', () => {
  it('resolves one exact capture across semantically unchanged rerenders', async () => {
    capture.mockResolvedValue({ format: 'png', data: 'AA==' })
    const view = render(
      <MaestroBrowserSurfaceCard
        nodeId="node-1"
        receipt={receiptFor('2026-08-24T12:00:00.000Z')}
        selected={false}
        onClick={vi.fn()}
    ...
    // The Canvas rebuilds its projection objects on every render; that must not restart capture.
    ...
    await waitFor(() =>
      expect(screen.getByAltText('Exact Browser evidence for Browser validation')).toBeTruthy()
    )
    expect(capture).toHaveBeenCalledOnce()
  })

  it('reports why the exact capture is unavailable', async () => {
    capture.mockRejectedValue(new Error('browser page not found'))
    render(
      <MaestroBrowserSurfaceCard
        nodeId="node-1"
        receipt={receiptFor('2026-08-24T12:00:00.000Z')}
    ...
    await waitFor(() => expect(screen.getByText('Capture unavailable')).toBeTruthy())
    expect(
      screen.getByText('Capture unavailable').closest('[data-maestro-preview-error]')
    ).toBeTruthy()
  })
})
