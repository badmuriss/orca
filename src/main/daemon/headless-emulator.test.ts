...
describe('HeadlessEmulator', () => {
  ...
  describe('write and snapshot', () => {
    ...
    it('proves shell ownership when command completion follows an unclosed alternate screen', async () => {
      ...
      await vi.waitFor(() => {
        expect(emulator.getSnapshot()).toMatchObject({
          modes: { alternateScreen: false, mouseTracking: false },
          terminalOwner: 'shell'
        })
      })
    })
    ...
    it('does not let later completion candidates starve a snapshot settlement', async () => {
      const confirmations: ((confirmed: boolean) => void)[] = []
      const confirm = vi.fn(
        () => new Promise<boolean>((resolve) => void confirmations.push(resolve))
      )
      emulator = new HeadlessEmulator({ cols: 80, rows: 24 })
      emulator.setShellOwnershipConfirmation(confirm)
      await emulator.write('\x1b[?1049hTUI\x1b]133;D;1\x07')
    ...
    it('normalizes proven shell modes once and avoids later process scans', async () => {
      const confirm = vi.fn(async () => true)
      emulator = new HeadlessEmulator({ cols: 80, rows: 24 })
      emulator.setShellOwnershipConfirmation(confirm)
      await emulator.write('\x1b[?1049h\x1b[?1003hTUI\x1b]133;D;137\x07')
      ...
      await emulator.write('\x1b]133;C\x07plain command\x1b]133;D;0\x07')
      await emulator.settleShellOwnershipConfirmation()
    ...
    it('revokes shell ownership when a live TUI enables kitty keyboard input', async () => {
      emulator = new HeadlessEmulator({ cols: 80, rows: 24 })
      emulator.setShellOwnershipConfirmation(async () => true)
      await emulator.write('\x1b[?1049hTUI\x1b]133;D;137\x07')
      await vi.waitFor(() => expect(emulator.getSnapshot().terminalOwner).toBe('shell'))

      await emulator.write('\x1b[>1uLIVE-TUI')
